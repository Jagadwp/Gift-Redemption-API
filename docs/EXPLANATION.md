...
- **Indexes for frequent access patterns**:
  - `idx_users_email` (partial index on `email` for non-deleted users)
  - `idx_gifts_created_at` and `idx_gifts_avg_rating` for listing/sorting
  - `idx_redemptions_user_id`, `idx_redemptions_gift_id` for lookup by user/gift
  - `idx_ratings_user_id`, `idx_ratings_gift_id` for aggregate stats
...
- **Constraint enforcement**:
  - `ratings.score` check constraint (1 to 5)
  - `uq_rating_redemption` unique constraint ensures one rating per redemption
